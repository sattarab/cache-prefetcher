	     ...
	     int prefetch_type)		/* prefetcher type */
{
  ...
  switch (prefetch_type)
  {
    ...
    case 2:
      ...
      ght = (history_table_entry*)malloc(num_lines * sizeof(history_table_entry));

      /* Initialize all the entries in the history table */
      for (index = 0; index <  num_lines; index++)
      {
        ght[index].pc = 0;
        ght[index].last_addr = 0;
        ght[index].last_prefetch = 0;
        ght[index].curr_delta = 0;
        ght[index].prev_delta = 0;
        
        int k;

        /* Initialize delta table entries for each history table entry */
        for (k = 0; k < MAX_DELTAS; k++)
        {
          ght[index].deltas[k] = 0;
        }
      }

      index_mask = num_lines - 1;
      tag_mask = log_base2(num_lines);
    ...
    default:
      ...
      rpt = malloc(sizeof(rpt_entry) * num_lines);

      /* Initialize all the entries in the rpt */
      /* reset index mask */
      tag_mask = 0;
      index_mask = 0;
...
void next_line_prefetcher(struct cache_t *cp, md_addr_t addr) {
  ...
  /* ECE552 Assignment 5 - next line prefetcher begin */

  /* add addr + cache line to the cache if not already present */
  ...
  /* ECE552 Assignment 5 - next line prefetcher end */
}

/* HELPER FUNCTIONS */

/* this helper function returns the most recurring element in the array */

/* ECE552 Assignment 5 - helper functions begin */

int get_common_element(int array[]) {
    int counter = 0;
    int current_common = array[0];
    int i;
    ...
    for (i = 0; i < MAX_DELTAS; ++i) 
    {
      if (counter == 0) 
      {
        current_common= array[i];
        ++counter;
      }
      else if(current_common == array[i]) 
      {
        ++counter;
      } 
      else 
...
/* this helper function does prefetch filtering on all the candidates */
int* prefetch_filtering(struct cache_t* cp, history_table_entry entry, int* candidates)
{
  int* prefetches = (int*)malloc(MAX_DELTAS * sizeof(int));
  int i;
  int index = 0;
  ...
  for (i = 0; i < MAX_DELTAS; i++)
  {
    ...
    {
      if (candidates[i] == entry.last_prefetch)
      {
        int j;

        for (j = 0; j < MAX_DELTAS; j++)
        {
          prefetches[i] = NULL;
        }
      }
      else if (!cache_probe(cp, entry.last_addr + candidates[i]))
      {
        prefetches[index] = candidates[i];
        index++;
      }
    }
  }
...
/* this helper function calculates delta correlation table for a given table entry */
int* delta_correlation(history_table_entry entry) 
{
  int* candidates = (int*)malloc(MAX_DELTAS * sizeof(int));
  ...
  if (entry.curr_delta == entry.prev_delta)
  {
    return NULL;
  }
  else
  {
    for (i = 0; i < MAX_DELTAS; i++)
    {
      if (i != entry.curr_delta && i != entry.prev_delta)
      {
        candidates[index] = entry.deltas[i];
        index++;
      }
    }
    return candidates;
...
/* ECE552 Assignment 5 - helper functions end*/

/* Open Ended Prefetcher */
void open_ended_prefetcher(struct cache_t *cp, md_addr_t addr) {

  /* ECE552 Assignment 5 - open ended begin*/

  unsigned int PC = (unsigned int)get_PC() >> 3;
  int index = PC & index_mask;
  unsigned int tag = PC >> tag_mask;

  if (ght[index].pc != tag)
  {
    ght[index].pc = tag;
    ght[index].last_addr = addr;
    ght[index].curr_delta = 0;
    ght[index].prev_delta = 0;
    ght[index].last_prefetch = 0;
  }
  ...
  {
    int delta_index = ght[index].curr_delta;

    ght[index].deltas[delta_index] = (unsigned int)addr - ght[index].last_addr;
    ght[index].last_addr = (unsigned int)addr;

    int* candidates = delta_correlation(ght[index]);
    int* prefetches = NULL;

    if (candidates != NULL)
    {
      prefetches = prefetch_filtering(cp, ght[index], candidates);
      free(candidates);
    }
 
    if (prefetches != NULL)
    {
      int data = get_common_element(prefetches);
      
      if (!cache_probe(cp, addr +data))
      {
        cache_access(cp, Read, addr + data, NULL, 1, (tick_t)0, NULL, NULL, 1);
      }
  
      free(prefetches);
    }
      
    ght[index].prev_delta = ght[index].curr_delta;
    ght[index].curr_delta = (ght[index].curr_delta + 1) % MAX_DELTAS;
  }
   
  /* ECE552 Assignment 5 - open ended end*/
}

/* Stride Prefetcher */
void stride_prefetcher(struct cache_t *cp, md_addr_t addr) {
  
  /* ECE552 Assignment 5 - strided begin */

  /* ignore the last three significant bits in PC */
  unsigned int PC = (unsigned int)get_PC() >> 3;
  int index = PC & index_mask;
  unsigned int tag = PC >> tag_mask;
  
  /* if  the tag is not in the rpt table */
  ...
  /* the tag is already in the rpt table */
  else
  {
    unsigned int stride =  (unsigned int)addr - (rpt[index].prev_addr);
    ...
    switch (rpt[index].state)
    {
      case INITIAL:
        ...
        {
          /* change the state from initial to transient */
          rpt[index].state = STEADY;
	      }
	      else 
        {
          /* update stride and change the state from initial to steady */
    ...
    if ((rpt[index].state == INITIAL || rpt[index].state == TRANSIENT|| rpt[index].state == STEADY)
  ...
  /* ECE552 Assignment 5 - strided end */
}
